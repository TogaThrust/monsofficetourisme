/**
 * Portes du Théâtre Royal : texte long réécrit (sans répéter le court),
 * traductions 10 langues, MP3 longs.
 * Usage: node scripts/fill-portes-theatre.mjs
 */
import fs from 'fs';
...
const FR_LONG_BODY = `Regardez le porche, ici sur la Grand-Place, à l’angle de la rue Neuve. Charles Sury, architecte de la Ville, dresse ce théâtre néo-classique entre 1841 et 1843. On l’appelle alors le Grand Théâtre ; le public y passe pour la première fois le 18 octobre 1843. Trois grilles ferment le passage, entre des colonnes ioniques. Ph.-J. Hoyois, ferronnier, les a façonnées. Émile-Joseph Hoyaux, sculpteur montois d’à peine vingt ans, y pose les visages en 1846. Au fronton : les attributs de la scène et de la musique.

Au XXe siècle, le bâtiment ne protège plus assez. Les travaux commencent en 1938 : la salle trop fatiguée disparaît, Govaerts et Van Varenberg en dessinent une nouvelle, inaugurée le 4 octobre 1948. Pour loger un escalier intérieur, plus sûr, plus commode, on conserve les grilles d’origine — mais on les coupe en hauteur. Elles ont changé de taille, pas de place.`;
...
const parsed = await chatJson(
  'You translate CityLoop Quest Mons visitor texts. Return JSON only. Keep proper names exactly: Charles Sury, Ph.-J. Hoyois, Émile-Joseph Hoyaux, Govaerts, Van Varenberg, Grand-Place, rue Neuve, Grand Théâtre, Mons. cn = Simplified Chinese. jp = Japanese.',
...
Do not list Molière, Racine, Grétry or Roland de Lassus.
Do not mention the coat of arms of Mons or that the doors are cast iron — those facts belong to the short text only.
...
fs.writeFileSync(path.join(ROOT, 'data', 'portes_du_theatre_royal.txt'), txt);
fs.mkdirSync(path.join(ROOT, 'dist', 'data'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'dist', 'data', 'portes_du_theatre_royal.txt'), txt);
...
for (const lang of LANGS) {
  const out = path.join(ROOT, 'audio', `PortesDuTheatreRoyal_${lang}.mp3`);
