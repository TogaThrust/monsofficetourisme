/**
 * Au Renard : texte long réécrit (sans répéter le court),
 * AV RENARD lu Au Renard en TTS, traductions 10 langues, MP3 longs.
 * Usage: node scripts/fill-au-renard.mjs
 */
import fs from 'fs';
...
const FR_LONG_BODY = `Le panneau forme l’allège de la fenêtre du milieu, au premier étage : un rectangle de calcaire, écorné, saillant sous l’appui. Maison d’angle, façade de pierre, classicisme montois. Le renard court vers la droite, queue touffue, gueule entrouverte. Il reste un peu de brun sur le flanc. En bas : AV RENARD. Le V vaut U. Puis le millésime.

La Maison du Renard est déjà nommée ici en 1510. Deux siècles plus tard, la pierre reprend le nom. On n’en voyait plus que le haut. En 1977, on l’a dégagée.`;
...
const parsed = await chatJson(
  'You translate CityLoop Quest Mons visitor texts. Return JSON only. Keep AV RENARD exactly as carved (V not U). Keep Maison du Renard. The stone is limestone (calcaire), the facade is stone (not brick). cn = Simplified Chinese. jp = Japanese. allège = stone panel under a window. millésime = the carved year, do not write 1724.',
...
Do not repeat the short-card facts: do not write 1724, do not write "51 rue d'Havré", do not open with "running fox in half-relief".
Do not mention Au Corbeau, À la Clef, the 1691 siege, or commercial cunning.
Keep AV RENARD unchanged in every language.
...
fs.writeFileSync(path.join(ROOT, 'data', 'au_renard.txt'), txt);
fs.mkdirSync(path.join(ROOT, 'dist', 'data'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'dist', 'data', 'au_renard.txt'), txt);
...
for (const lang of LANGS) {
  const out = path.join(ROOT, 'audio', `AuRenard_${lang}.mp3`);
