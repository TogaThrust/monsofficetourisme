/**
 * Bump version + SW + notes after fill-grand-laboureur.mjs.
 * Lit les numéros RÉELS, +1. Usage: node scripts/bump-grand-laboureur.mjs
 */
import fs from 'fs';
...
const block =
  ...
  `Version ${nextVer}\r\n` +
  '----------------------------------------\r\n' +
  'Au Grand Laboureur : texte long réécrit (sans répéter le court), Clef lu Clé en TTS, 10 langues + audio long.\r\n\r\n';
...
copyViaTmp(
  path.join(ROOT, 'data/au_grand_laboureur.txt'),
  path.join(ROOT, 'dist/data/au_grand_laboureur.txt'),
);
copyViaTmp(
  path.join(ROOT, 'images/Au_Grand_Laboureur.jpg'),
  path.join(ROOT, 'dist/images/Au_Grand_Laboureur.jpg'),
);
