/**
 * Bump version + SW + notes after fill-croix-or-croix-place.mjs.
 * Lit les numéros RÉELS, +1. Usage: node scripts/bump-croix-or-croix-place.mjs
 */
import fs from 'fs';
...
const block =
  ...
  `Version ${nextVer}\r\n` +
  '----------------------------------------\r\n' +
  "A la Croix d'Or Croix-Place : texte long réécrit (sans répéter le court), 10 langues + audio long.\r\n\r\n";
...
copyViaTmp(
  path.join(ROOT, 'data/a_la_croix_dor_croix-place.txt'),
  path.join(ROOT, 'dist/data/a_la_croix_dor_croix-place.txt'),
);
copyViaTmp(
