/**
 * Bump version + SW + notes after fill-colombe-saint-esprit.mjs.
 * Lit les numéros RÉELS, +1. Usage: node scripts/bump-colombe.mjs
 */
import fs from 'fs';
...
const block =
  ...
  `Version ${nextVer}\r\n` +
  '----------------------------------------\r\n' +
  'Colombe du Saint-Esprit : texte long réécrit (sans répéter le court), 10 langues + audio long. TTS : ein / s’invole / infants (patois Gillis).\r\n\r\n';
...
const img = path.join(ROOT, 'images/Colombe_du_Saint-Esprit.jpg');
if (fs.existsSync(img)) {
  fs.mkdirSync(path.join(ROOT, 'dist/images'), { recursive: true });
  copyViaTmp(img, path.join(ROOT, 'dist/images/Colombe_du_Saint-Esprit.jpg'));
}
