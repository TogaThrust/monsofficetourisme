/**
 * Le Lecteur Colas : texte long réécrit (sans répéter le court),
 * traductions 10 langues, MP3 longs.
 * Usage: node scripts/fill-lecteur-colas.mjs
 */
import fs from 'fs';
...
const FR_LONG_BODY = `Un homme, à mi-corps, sort du mur. Tête baissée, un livre ouvert trop grand, pages nues. Pierre brute, plis lourds, comme un manteau. Le cartouche, lui, est plus vieux : calcaire gris, coins écornés en quart-de-rond, mutilé. Il tient le registre, entre la vitrine et l’étage. Quelque chose y a disparu ; le lecteur a pris la place.

André Leto tenait librairie ici. Pas de néon, pas de panneau. Une sculpture, pour dire les livres, et pour parler comme les enseignes de pierre de la ville. Le sculpteur taille le personnage dans le même matériau que le cadre.

Les vitrines changent. Le lecteur n’a pas bougé. Yannart l’a rangé parmi les enseignes remarquables. Levez les yeux au-dessus de la devanture : il lit encore.`;
...
const parsed = await chatJson(
  'You translate CityLoop Quest Mons visitor texts. Return JSON only. Keep proper names exactly: André Leto, Yannart, Le Lecteur, Mons. The sculpture is stone (pierre), not bronze. cn = Simplified Chinese. jp = Japanese.',
...
Do not repeat the short-card facts: do not write 1990, do not write "35 rue d'Havré", do not open with "modern sculpture by Colas placed on an old cartouche".
...
fs.writeFileSync(path.join(ROOT, 'data', 'le_lecteur_colas.txt'), txt);
fs.mkdirSync(path.join(ROOT, 'dist', 'data'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'dist', 'data', 'le_lecteur_colas.txt'), txt);
...
for (const lang of LANGS) {
  const out = path.join(ROOT, 'audio', `LeLecteurColas_${lang}.mp3`);
